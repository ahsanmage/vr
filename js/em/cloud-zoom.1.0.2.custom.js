...
/* Add a box (mouseTrap) over the small image to trap mouse events.
		...
		*/
			...
			$mouseTrap.bind(m_ent, this, function(event) {
				...
				if (navigator.userAgent.match("MSIE") && $.browser.version < 7) {
				...
				if (cw > event.data.zoomDivWidth) cw = Math.min(sImg.outerWidth(), event.data.zoomDivWidth);
				ch = (sImg.outerHeight() / zoomImage.height) * event.data.zoomDivHeight;
				if (ch > event.data.zoomDivHeight) ch = Math.min(sImg.outerHeight(), event.data.zoomDivHeight);
